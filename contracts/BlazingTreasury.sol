...
contract BlazingTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SBLAZ }
    ...
    address public sBLAZ;
    uint public sBLAZQueue; // Delays change to sBLAZ address
    ...
    constructor (
        ...
        isLiquidityToken[ _BLAZDAI ] = true;
        liquidityTokens.push( _BLAZDAI );
        bondCalculator[_BLAZDAI] = _bondCalculator;

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for BLAZ
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint BLAZ needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( BLAZ ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn BLAZ for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOfToken( _token, _amount );
        IBLAZERC20( BLAZ ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sBLAZ ).balanceOf( msg.sender ); // Can only borrow against sBLAZ held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with BLAZ
        @param _amount uint
     */
    function repayDebtWithBLAZ( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IBLAZERC20( BLAZ ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, BLAZ, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( BLAZ ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( BLAZ ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns BLAZ valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOfToken( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match BLAZ decimals
            value_ = _amount.mul( 10 ** IERC20( BLAZ ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyOwner() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SBLAZ ) { // 9
            sBLAZQueue = block.number.add( blocksNeededForQueue );
        } else return false;
